...
| `GET /api/threads/{thread_id}/workspace-files` | 当前任务文件 | 分别列出线程 `inputs/` 研究资料与 `outputs/` 分析产出。 |
| `GET /api/threads/{thread_id}/file-preview/*` | 文件预览 | 在路径隔离与同线程鉴权下提供表格及受限内嵌内容预览。 |
...
- 地图是空间证据视图，不是装饰性 KPI 仪表盘；主工作台监测地图默认以中国为中心、使用亚太至全球范围，专题页面可切换到研究区尺度。
- 用户任务进度只显示在该用户的研究对话中；全局监测状态只显示在公共地图与监测队列中。
- 四个研究角色的可显示文本通过 SSE 按角色增量呈现；工具状态与最终研究结论仍分别保留在任务进程和正式对话记录中。
- 右侧工作区默认显示监测面板；项目文件和文件预览作为并列标签按需打开。桌面端宽度可拖拽或用键盘调整，折叠后应恢复用户设置；窄屏下改为自然堆叠，不提供拖拽柄。
