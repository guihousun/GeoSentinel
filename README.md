...
| 可控执行 | 按版本确认方案、角色工具白名单、自动排队、取消、持久化运行记录 |
...
完整 NTL/Earthdata/MCP 工具覆盖、国家专题、企业知识库、个人 GEE 账号绑定及跨机分布式调度仍需逐项迁移和验收。当前已有单进程持久化排队、可配置并发与用量管理，见 [资源与队列管理](dsh/RESOURCE-MANAGEMENT.md)。不要把旧 Python 工具清单当作新版已开放能力清单。
