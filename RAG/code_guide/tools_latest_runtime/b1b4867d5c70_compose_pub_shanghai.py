...
"""
Task: 生成 pub_shanghai.tif —— 上海市（经度 121.2-121.6，纬度 30.9-31.3）2022-02-01 至 2022-02-02
      VNP46A2 每日夜间灯光（ANTL）两日均值合成。

Inputs (verified staged daily layers, previously acquired from GEE):
  - inputs/shanghai_vnp46a2_2022-02-01.tif   (VNP46A2 daily, float64, EPSG:4326, nodata=-Inf)
  - inputs/shanghai_vnp46a2_2022-02-02.tif   (VNP46A2 daily, float64, EPSG:4326, nodata=-Inf)

Declared outputs:
  - outputs/pub_shanghai.tif                 (mean composite, float32, NaN nodata, EPSG:4326)
...
NTL_SCRIPT_CONTRACT = {
    ...
    "objective": "生成上海市（121.2-121.6E, 30.9-31.3N）2022-02-01 至 2022-02-02 VNP46A2 每日夜间灯光两日均值合成 outputs/pub_shanghai.tif",
    "input_manifest": [
        {"path": "inputs/shanghai_vnp46a2_2022-02-01.tif", "role": "daily_source", "media_type": "image/tiff"},
        {"path": "inputs/shanghai_vnp46a2_2022-02-02.tif", "role": "daily_source", "media_type": "image/tiff"},
    ],
    "method_steps": [
        "读取两个逐日 VNP46A2 图层（float64, EPSG:4326, nodata=-Inf）并校验网格对齐",
        "逐像素有效值均值合成（-Inf/NaN 视为无效；仅单日有效取该日值；均无效为 NaN）",
        "写出 outputs/pub_shanghai.tif（float32, NaN nodata, 与输入同 geotransform/projection）",
    ],
    "parameters": {
        "reducer": "mean",
        "dtype_out": "float32",
        ...
        "product": "VNP46A2 standard values",
    },
    "output_manifest": [
        {"path": "outputs/pub_shanghai.tif", "role": "result", "media_type": "image/tiff"},
    ],
    "validation_checks": [
        "output 存在且可读，形状 90x90，CRS EPSG:4326，边界与输入一致",
        "输出均值与逐日输入均值之平均一致（容差 1e-6）",
...
INPUT_PATHS = [
    "inputs/shanghai_vnp46a2_2022-02-01.tif",
    "inputs/shanghai_vnp46a2_2022-02-02.tif",
]
OUTPUT_PATH = "outputs/pub_shanghai.tif"
...
print("COMPLETED pub_shanghai.tif")
