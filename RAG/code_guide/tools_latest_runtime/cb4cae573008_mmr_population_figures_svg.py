...
# Panel B: 2000-2020 change per unit (divergent bars from a shared zero line at panel centre)
col2_x = ML2 + int(plot_w2 * 0.62)
col2_plot_w = plot_w2 - int(plot_w2 * 0.62) - 20
half_w = col2_plot_w * 0.5
max_abs = max(abs(r["change"]) for r in rows_sorted) * 1.15
zero_x = col2_x + half_w
p2.append(f'<text x="{col2_x}" y="{MT2+2}" font-size="13" font-weight="bold" font-family="sans-serif" fill="#1a1a1a">2000-2020 % change</text>')
p2.append(f'<line x1="{zero_x:.1f}" y1="{MT2+8:.1f}" x2="{zero_x:.1f}" y2="{MT2+18+14*(bar_h+gap)+4:.1f}" stroke="#999" stroke-width="1"/>')
for i, r in enumerate(rows_sorted):
    y_top = MT2 + 18 + i * (bar_h + gap)
    w = abs(r["change"]) / max_abs * half_w
    col = "#27ae60" if r["change"] >= 0 else "#c0392b"
    if r["change"] >= 0:
        p2.append(f'<rect x="{zero_x:.1f}" y="{y_top:.1f}" width="{w:.1f}" height="{bar_h:.1f}" fill="{col}"/>')
        p2.append(f'<text x="{zero_x+w+5:.1f}" y="{y_top+bar_h-3:.1f}" font-size="12" font-family="sans-serif" fill="#1a1a1a">+{r["change"]:.1f}%</text>')
    else:
        p2.append(f'<rect x="{zero_x-w:.1f}" y="{y_top:.1f}" width="{w:.1f}" height="{bar_h:.1f}" fill="{col}"/>')
        p2.append(f'<text x="{zero_x-w-5:.1f}" y="{y_top+bar_h-3:.1f}" text-anchor="end" font-size="12" font-family="sans-serif" fill="#1a1a1a">{r["change"]:.1f}%</text>')
