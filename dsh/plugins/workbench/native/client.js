      ...
      function ReleasePanel() {
        ...
        return h("div", { className: "geo-release-panel" }, h("h2", null, "产品配置与发布"),
          ...
          error && h("p", { role: "alert" }, error), !data ? h("p", { role: "status" }, "正在读取版本…") : h(React.Fragment, null,
            ...
            data.phase && h("p", { role: "status" }, ({ checking: "检查环境中", building: "构建并测试中", previewing: "启动预览中", accepting: "正在验证上传与智能体读取", ready: "预览就绪", failed: "生成预览失败" })[data.phase] || ""),
            ...
            h("div", { className: "geo-release-actions" },
              button(data.preparing ? "正在验证…" : "生成预览", icons.IconRefreshOutline16, act("prepare"), { disabled: busy || data.preparing || edited || Boolean(data.pending) }),
              button("发布此版本", icons.IconPlayOutline16, act("publish", { id: data.candidate?.id }), { disabled: busy || !data.supervised || data.candidate?.status !== "validated" || data.candidate?.acceptance !== "passed" || Boolean(data.pending) })),
