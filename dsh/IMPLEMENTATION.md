...
| Admission queue and cancellation | bounded active chats per account; cancelling stops a run | same, re-verified on the current candidate | ordinary-user run through the product's routes: with three chats prompted, the third answered `queued: true` and its queue snapshot read `running=0 waiting=1` while the other two ran; `POST /api/chats/<id>/cancel` returned 200 and that chat stopped within ~5 s (events 33 → 38); the queued chat then took its turn (`waiting=1 → running=1`) |
