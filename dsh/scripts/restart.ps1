...
foreach ($listener in $listeners) {
    ...
    if (!$server -or $server.Name -ne 'node.exe' -or !$server.CommandLine.Contains($releaseState.directory) -or !$server.CommandLine.Contains($expectedProfile)) { throw "Port $Port belongs to a different service; nothing stopped." }
