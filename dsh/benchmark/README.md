...
- `errors` 去重且只扫描每轮新增事件；此前每次轮询重复记录同一个 `tool-error`。`tool-error` 会带上**工具名**（按 `tool/call` 的 `callId` 关联；`native-history` 本身不暴露失败工具），只说明某次工具调用失败（模型常常随后重试成功），判分不使用 `errors`。报告另记 `turns`/`steps` 与每次解锁消息的 `seq`（形如 `unblock-prompt:1@seq42`），用来区分"模型提前停手被催促"和"确实耗尽预算"。
