...
- **A04 预算偏紧（最终候选全量跑批发现）**：该例要取震前、震后各 7 天的日尺度灯光再比较，
  45 分钟不够：同一用例在上一轮 2708s 处勉强写完并通过，这一轮 2710s 处预算耗尽、窗口结论没写出来。
  计时器而不是能力决定了结果，按 B02 的先例把预算从 **45 分钟调到 60 分钟**
  （与 A01/A09/C10 同档）。
...
## 9. 最终候选全量重跑（`9f829af5881d642f`）

D10/D11/D12 修完、用例与判分缺陷也修完后，在**同一个最终候选**上又跑了一遍全量 31 例
（`--concurrency 4`，原始记录 `dsh/.runtime/benchmark-report-full-9f829af5.json`，
本地 07:24→08:51，墙钟 87 分钟，单例耗时之和 20,283s）。这一轮用的是**修正后的用例与判分器**，
不需要事后重算：

| 用例 | 档位 | 检查 | 耗时 | 说明 |
| --- | --- | --- | --- | --- |
| B01 上海 2020 年分区 ANTL 统计 | 基础 | 6/6 | 687s | 上一候选曾 5/6，本轮完整通过（D10 现场） |
| B02 单幅栅格质量检查 | 基础 | 3/3 | 515s | |
| B03 中国省级边界获取与核验 | 基础 | 4/4 | 288s | |
| B04 逐年 ANTL 时间序列 | 基础 | 3/3 | 470s | 上一候选曾 2/3，本轮完整通过（D10 现场） |
| B05 等距缓冲区 | 基础 | 4/4 | 222s | |
| B06 按多边形筛选点 | 基础 | 2/2 | 404s | |
| B07 栅格重投影 | 基础 | 2/2 | 642s | |
| B08 相邻栅格镶嵌 | 基础 | 2/2 | 521s | |
| B09 指标口径解释 | 基础 | 2/2 | 309s | |
| B10 中文可视化 | 基础 | 3/3 | 1259s | |
| A01 分区统计 + 证据链 | 进阶 | 6/6 | 308s | |
| A02 首夜日期与 UTC 换算 | 进阶 | 2/2 | 1510s | |
| A03 月度时间序列选型 | 进阶 | 3/3 | 603s | |
| A04 事件窗口对比 | 进阶 | 4/6 | 2710s | 预算用尽（45 分钟），未写到震前/震后窗口 → 见 §4 预算修正 |
| A05 异常检测 | 进阶 | 3/3 | 719s | |
| A06 多源叠加分析 | 进阶 | 3/3 | 524s | |
| A07 两套边界差异 | 进阶 | 3/3 | 469s | |
| A08 全球事件监测线索核验 | 进阶 | 4/4 | 458s | |
| A09 多角色综合报告 | 进阶 | 5/5 | 1207s | |
| A10 点与行政区空间连接 | 进阶 | 1/2 | 270s | 未调用 `geo_spatial_join_points_to_admin`（改用自写脚本），见下 |
| A11 共享边界库取用与来源声明 | 进阶 | 6/6 | 148s | |
| C01 超大范围下载的单次上限与失败语义 | 挑战 | 5/5 | 1536s | 修正后的期望一次通过 |
| C02 时区陷阱 | 挑战 | 5/5 | 297s | |
| C03 跨传感器可比性 | 挑战 | 5/5 | 1184s | |
| C04 趋势显著性 | 挑战 | 5/6 | 762s | 未调用 `geo_analyze_ntl_trend`（改用指标工具＋自写脚本），见下 |
| C05 归因边界 | 挑战 | 5/5 | 633s | |
| C06 海域覆盖与掩膜声明 | 挑战 | 6/6 | 219s | |
| C07 来源冲突记录 | 挑战 | 3/3 | 239s | |
| C08 精度与空间支持声明 | 挑战 | 5/5 | 142s | 修正后的期望一次通过 |
| C09 重复运行不等于独立验证 | 挑战 | 6/6 | 330s | 否定句判分修正后一次通过 |
| C10 端到端地缘事件分析 | 挑战 | 9/9 | 698s | |

**结果：28/31 通过**（基础 **10/10**、进阶 9/11、挑战 9/10）。三个未过项都不是平台缺陷：

- **A04（预算用尽）**：该例本质要取两个 7 天窗口的日尺度数据再比较，45 分钟不够；
  同一用例在上一轮 2708s 处勉强收口、这一轮 2710s 处没写完。这是执行器时限，不是能力结论——
  按 B02 的先例把预算调到 **60 分钟**（与 A01/A09/C10 一致），见 §4。
- **A10 与 C04（未调用已注册工具）**：两例都改用自写脚本完成同类计算，
  因此缺少 `geo_spatial_join_points_to_admin` / `geo_analyze_ntl_trend` 的调用记录。
  核查证据：会话里**没有**任何"该工具在本平台不可用"的守卫拒绝，两个工具在同一天的其它用例中
  都被正常调用（A10 在上一轮 2/2、B04 本轮调用了 `geo_analyze_ntl_trend`），
  且 `code-execution-validation` 技能已明确写着"平台已注册的方法（……分区统计、趋势、……空间连接、融合）
  必须用对应工具，不要自己重写"。这是模型选路波动，不是平台缺陷；
  可选的下一步是**把这句话前移到工具描述里**（`geo_execute_python` 与趋势/异常工具的说明），
  但那属于产品源码改动，留到发布之后再评估。

**D10 在全量层面的复核**：B01 的调用序列里再次出现
`geo_inspect_raster(path="share/…", source="share")` 与 `geo_calculate_zonal_statistics`，
B04 同样回到注册的分区统计工具；两者都完整通过。上一轮这两个用例的失败（5/6、2/3）由此闭合。
...
- 累计定位 **12 个系统性缺陷（D1–D12）**，全部与模型能力无关，全部已修并有测试/证据：
  ...
  **D10（共享数据只在容器挂载里能用一半）、D11（候选验证过的镜像从未被运行实例使用）、
  D12（容器被杀只剩一句退出码）**。
- 修复效果的验证链：D10/D11 先在候选 `9f829af5881d642f` 上用例级复跑确认（§8：
  B01 6/6、B04 3/3、A11 6/6；容器镜像 77/77 命中候选摘要），随后该候选**全量 31 例**
  跑出 28/31（§9），B01/B04 的 D10 现场关闭、D12 仍是单测级证据。
- 用例集从 30 例扩到 **31 例**（新增 A11）；累计修正 6 处用例/判分缺陷：C06 前提、B02 预算、
  C01 禁止词、C08 关键词、C09 否定句判分（§4）、A04 预算（45→60 分钟）。
- 剩余三个未过项都不是平台缺陷：A04 是预算（已调），A10/C04 是模型没调用已注册工具（守卫拒绝 0 次、
  同类工具在其它用例正常调用）；可选后续改进是把"注册方法必须用对应工具"这句话从技能前移到工具描述，
  那属于产品源码改动，**发布之后**再评估。
- 下一步：① 生成新预览与验收，发布已授权的候选 `9f829af5881d642f`；② 发布后在正式实例上抽验
  B01/B04/A11 与健康状态里的版本 ID；③ 观察 D12 在真实 OOM/超时场景下的提示。

## 11. 限制

- 全量与各批次均为每例一次，**不构成独立验证**；规则分不等于科学质量评阅。
- 同一用例在不同轮次会走不同路线（B01/B04 曾因 D10 少调一次工具、A10/C04 本轮改用自写脚本），
  因此"用例是否通过"不能单独当作能力结论，必须结合工具调用序列与错误明细看。
