...
- 该版本上共定位 **9 个系统性缺陷（D1–D9）**，全部与模型能力无关，全部已修并有测试/证据：
  ...
  随后在第二批 6 例上完成 D1–D8 的整批复跑（24/24 全通过）。D9 是本轮复查发现的 D1 回归，
  修在冻结之后，因此 `a01d2f98a7e9a206` 已过期。
- 用例集从 30 例扩到 **31 例**（新增 A11），并修正了两处用例缺陷（C06 前提、B02 预算）。
- 下一步：① 重新冻结一个含 D9 的候选，并在该候选上跑**全量 31 例**，得到单一版本的完整结果；
  ② 全量结果出来后重新生成预览与验收，再请管理员授权发布；③ 发布后在正式实例上抽验。
...
- 各批次均为每例一次，**不构成独立验证**；规则分不等于科学质量评阅。
- 修复验证只覆盖曾受影响的用例；D4/D6 的运行时效果需要真实场景再现才能确认。
