...
| B04 逐年 ANTL 时间序列 | 基础 | 多期取数 + 时序一致性 | 运行中 |
| A05 异常检测 | 进阶 | `geo_detect_ntl_anomaly` + 阈值口径 | 运行中 |
| A07 两套边界差异 | 进阶 | 两套边界的差异与登记（共享库相关） | 运行中 |
| C01 超大范围下载失败语义 | 挑战 | 失败语义（D2 相关的 GEE 提示） | 运行中 |
| C04 趋势显著性 | 挑战 | `geo_analyze_ntl_trend`（上轮唯一未调用的工具） | 运行中 |
...
- 该版本上共定位 **8 个系统性缺陷（D1–D8）**，全部与模型能力无关，全部已修并有测试/证据：
...
- 用例集从 30 例扩到 **31 例**（新增 A11），并修正了两处用例缺陷（C06 前提、B02 预算）。
- 下一步：① 发布候选 `a01d2f98a7e9a206`（需管理员授权）；② 发布后按同一子集做完整
  8+1 例复跑，补齐 A10/A11 在**正式实例**上的表现；③ 观察 D8 的错误提示在真实触发时的效果。
