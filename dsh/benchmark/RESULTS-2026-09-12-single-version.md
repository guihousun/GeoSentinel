...
- 下一步：① 重新冻结含 D9–D12 的候选；② 在该候选上重跑 B01/B04（D10 现场）与 A11/A09（回归），
  并用 `docker inspect` 核对作业容器用的镜像摘要等于 `validation.json` 的摘要（D11 的直接证据）；
  ③ 重新生成预览与验收后，请管理员授权发布；④ 发布后在正式实例上抽验。

## 9. 限制

- 全量与各批次均为每例一次，**不构成独立验证**；规则分不等于科学质量评阅。
- D10/D11 的修复发生在该轮跑批之后：B01/B04 的失败**正是**这两个缺陷的现场，
  修复效果需要在下一个冻结候选上重跑同一用例确认（见 §8 下一步）。
- D12（容器被杀的可读原因）在本轮只以单测固定；现场触发需要再次出现 OOM/超时。
