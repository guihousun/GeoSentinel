...
- 单用户同时一个运行中的研究对话，全局最多八个；Docker 单用户同时一个作业、全局同时两个。每作业默认 10 分钟、256 MiB 输出监测限额。输出限额是轮询终止策略，并非文件系统硬配额。
- 上传暂限 16 MiB/文件、512 MiB/项目；账号最多 100 个项目、每项目 200 个对话；每账号每分钟最多提交 20 次问题。大规模数据处理需要下一步单独配置容量与排队。
- 删除为软删除，访问入口关闭但管理员存储仍保留；不是磁盘安全擦除。生产备份须同时覆盖整个独立 DSH home 和平台数据目录。
- 此版本是单服务进程的受控试点。Docker 不等于无条件安全沙箱；公网运行仍需限流、磁盘配额、备份、漏洞更新与运维监控。
