...
export function releaseService({ source, directory, runtime, store, extraIdle = () => true }) {
  ...
  let preparing = false, previewChild, preview, readySent = false, closing = false, mutations = 0, operationError;
  ...
  return {
    ...
    async status() { const status = await manager.status(); return { ...status, lastError: operationError || status.lastError, preparing, loadedRelease: process.env.GEO_RELEASE_ID ?? null, supervised: Boolean(process.send), previewUrl: preview?.expires > Date.now() ? preview.url : null }; },
    saveProduct: (product) => manager.saveProduct(product),
    prepare(by) {
      if (preparing) throw new Error("正在验证版本");
      preparing = true; operationError = null;
      void manager.prepare(by).catch((error) => { operationError = error.message; }).finally(() => { preparing = false; });
      return { accepted: true };
    },
    async preview(id) {
      if (preparing) throw new Error("版本正在验证，稍后再预览");
      ...
      if (preview?.id === id && preview.expires > Date.now()) return { url: preview.url };
      ...
      state.candidate.previewed = true; await manager.save(state);
    ...
    async publish(id, by, rollback) {
      ...
      readySent = false;
      return manager.request(id, by, rollback);
    },
