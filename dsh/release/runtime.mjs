import { mkdirSync, writeFileSync, symlinkSync, lstatSync, realpathSync } from "node:fs";
...
export async function launchRelease(manager, id, { home, args, preview, env = process.env }) {
  ...
  for (const [name, target] of [["@deepseek-ai/dsh-base", path.join(dsh, "node_modules/@deepseek-ai/dsh-base")], ["@deepseek-ai/dsh-web-app", path.join(dsh, "node_modules/@deepseek-ai/dsh-web-app")], ["@nanmicoder/dsh-agent-teams", path.join(app, "agentteams")], ["@changfenhuang/dsh-genui", path.join(dsh, "node_modules/@changfenhuang/dsh-genui")], ["dsh-file-upload", path.join(dsh, "node_modules/dsh-file-upload")], ...["platform", "research", "workbench"].map((name) => ["@geosentinel/dsh-" + name, path.join(dsh, "plugins", name)])]) {
  ...
  writeFileSync(path.join(profile, "cordis.patch.yml"), YAML.stringify(rows));
