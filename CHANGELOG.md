...
- **仍存的缺口**：① 原生会话列表的分组交互（重命名/归档/拖拽排序/搜索）由 `ui-workspace` 提供，产品用自己的列表替代后这些交互仍是产品自己的实现（管理入口在行的齿轮按钮里）；② 侧栏文件与文档预览（`ui-sidebar-files` / `-documentpreview`）等 `remote.workspaceFiles`，仍在封闭平面之外，产品此前的三组文件面板在 0.1.5 上未恢复；③ 管理员开发实例在 0.1.5 上的端到端实测要等主树升级后才有条件做（该实例的客户端组合与产品外壳不同）；④ 正式发布尚未执行，生产仍是 `9e061099f3ccf519`。
- 单元测试 97 项通过（主树与 0.1.5 副本各一遍）；`prepare → preview` 在副本内通过；候选 `f4cec4cb35fd2b40` 的客户端 bundle 与已验证的 `b855817f18abe1ee` 逐字节相同（`sha256=9a49221d…`，9265336 字节），因此其上的侧栏与面板复验结论同样成立；生产版本未改动。
