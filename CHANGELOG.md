...
- scripts/restart.ps1统一8511默认端口、版本和进程归属预检、后台启动及健康检查；旧临时命令仅转发。
- 历史发布版本的兼容启动并入release/compatible-runtime.mjs，启动前验证快照，不再依赖临时控制器实现。


## 2026-09-11 — 页面与 Agent 共用发布校验

- 页面精简为生成预览与发布此版本；环境检查、冻结测试和8513随机上传口令验收自动串联，显示阶段进度。
- CLI新增generate-preview，可携带本次用户授权执行自动发布；候选授权、验收证据与控制器兼容检查统一落在发布入口，控制器在停服前再次验证。
- 缺少验收、源码变动、验收过期及旧控制器不兼容时拒绝发布；不会自动替换线上旧控制器。


## 2026-09-11 — DSH 开发运行时升级修复

- 开发依赖统一锁定为 0.1.5-rc.1，补齐九个新增内部包的 overrides，避免混入 rc.2。
- 升级工具使用独立参数调用默认 Node 校验脚本，修复 Windows 含空格路径导致安装后误回滚；环境校验目标同步为 0.1.5-rc.1。
- 升级回归覆盖默认校验路径与失败回滚；133 项测试通过。正式发布快照仍需独立验证和发布。
...
- **发布前预演：从主树冻结即可产出与已验证候选**同字节**的版本**。候选 id 是冻结文件的哈希（`release/manager.mjs`：`id = sha(JSON.stringify(fileHashes)).slice(0,16)`），所以 id 相同即内容相同。实测：在临时 home（`GEO_DSH_HOME=.runtime/preflight-home`，不碰生产 home、也不动主树 `node_modules`）用主树跑 `prepare`，**一次通过**——离线安装用 `--frozen-lockfile`（说明主树的 pin 与锁文件已自洽，不会出现 `ERR_PNPM_OUTDATED_LOCKFILE`）、语法检查、冻结树内测试与 Docker 镜像全部成功，产出的 id 正是 `c5f518a3c6f651e6`，与 0.1.5 副本上验收过的那一份完全一致。也就是说：**发布这件事在技术链路上已经走通**，剩下的门只有两道——`RELEASES.md` 要求的"主树依赖升级"（会让当前管理员会话所用的运行时被替换，需要明确许可）与正式的发布授权。
- **验收做成一键套件并全过（6/6）**：`.runtime/acceptance-suite.py <port> <token>` 按顺序驱动既有的六个脚本并汇总判定，发布时一条命令即可复现同一份证据。本轮在同一候选上跑出 **6/6 PASS**：原生文件面板 + Markdown 预览、界面上传（坞/桥/工作区/账号用量）、PDF 文档预览、上传件进入研究流程（10 秒内 `read_document` 并答出口令）、文档预览自动刷新（约 4.0 秒）、分步方案 + 提问确认——**每一项都是 0 控制台错误、0 失败请求**。顺带看到主管在这一轮里用了 `exit_plan_mode`、`ask_user_question` 以及两次 `validate_dsh_ui`（产品自己的界面渲染校验工具确实在用）。
- **对等清单与"刻意边界"写进实现文档，并给候选一致性做了工具**：新增 `dsh/IMPLEMENTATION.md`「0.1.5 core migration: parity index and verification evidence」——逐面列出 0.1.2 产品与 0.1.5 候选的对照、证据（候选 id + 验收脚本 + 结果）与**刻意不复用**的理由；同时新增 `.runtime/candidate-validity.mjs`，随时回答"当前源码是否仍冻结出已验证的候选"（发布时 `publish` 自己也会做同样的比较）。实测：核对主树源码与冻结清单 **187/187 文件一致**，且**写完这份文档之后再核对仍然一致**——因为 `dsh/*.md` 不在冻结范围内（冻结只收 `plugins/profile/skills/monitoring/docker/release/development/scripts/cli/tests` 与三个单文件），所以文档类改动不会让候选失效。
- **顺带纠正一处措辞：会话列表的原生分组交互是刻意边界，不是对等缺口**。证据：`dsh-client-ui-workspace`（原生会话列表的持有者）的激活等待 `workspaces` 与 `remote.directoryPicker`，后者位于产品刻意封闭的主机平面——引导它会让加载器条目 pending，原生加载器随即把**整个**客户端 bundle 判为失败（白屏）；而且它**两代产品都没有被打包**：在跑的 0.1.2 冻结版本里根本没有它，模块级机械比对也只发现 `dsh-better-sidebar` 被移除。因此重命名/归档以产品自己的入口（行内齿轮、项目级归档）承载，拖拽排序与分组搜索从未是产品能力，不再列为待办。
- **三块产品面板在当前候选上复验（`c5f518a3c6f651e6`）**：侧栏底部的「全球事件监测 / 监测简报 / 空间数据」逐个打开，三块面板都**渲染出来了**——监测面板带关注等级与时间窗筛选、Natural Earth 底图、冲突/地缘政治/地震等类别图例与分级规则说明；简报面板给出「0 条简报 · 共 0 条线索 · 数据待更新」；空间数据面板给出文件计数与底图。`/geo/api/monitor/events` 两次 200、**控制台错误 0、失败请求 0**。**如实说明数据为空的原因**：预览实例按设计关闭监测采集（见 RELEASES.md），所以面板走的是空快照分支，并且它**如实说"数据待更新"、没有编造内容**；带真实数据的快照是在同一内核线的更早候选上验收过的（对等清单里已按此措辞记录）。这也顺手补掉了对等清单里唯一一句"未在当前候选重复验收"。
- **科研核心能力在当前候选上端到端实测（`c5f518a3c6f651e6`，普通用户，全走产品自己的路由）**：① **GEE 真下载**——主管在 5 秒内调用 `geo_download_gee`，产出 `分析结果/空间数据/imagery.tif`（8,952 字节）并留下作业记录 `过程记录/20260911-025409-gee-4c008a/{imagery.tif,result.json}`；② **容器真执行**——随后调用 `geo_execute_python`（约 30 秒内），产出 `ntl_summary.py`（1,262 字节）与 `ntl-summary.json`（139 字节）及自己的作业记录；③ **产物回读**（用产品自己的 `fs.tree` + `/sidebar/file`）：`{"dataset":"NOAA/VIIRS/001/VNP46A2","path":"outputs/20260911-025409-gee-4c008a/imagery.tif","pixels":2070,"mean":7.420264}`——数据集标识、产物路径、像元数与有效值均值都在，单位 nW/cm²/sr（0.2° 框 @500 米得到 2070 个有效像元，量级自洽）。没有 mock：跑的是部署的 GEE 入口与部署的分析镜像。这一轮的工具轨迹为 `geo_download_gee → web_search → geo_inspect_raster → read → validate_dsh_ui → skill → geo_execute_python`，并且主管把结论渲染成了产品的界面卡片。工作区列表同时显示管理员的只读共享库（缅甸地理）已按设计挂载。**没有控制台错误**（全程接口驱动，无页面导航）。对等清单里"科研核心"一行据此从"0.1.2 线验证过"升级为"当前候选实测"。
- **运行时行为在当前候选上复验：准入队列与取消（`c5f518a3c6f651e6`，普通用户）**：用产品自己的路由开三个研究对话并各自发一条"慢任务"提示——第三个立即返回 `queued: true`，稍后它的队列快照为 `running=0 waiting=1`，另两个正在运行，即**每用户活跃对话上限如实生效**；对其中一个发 `POST /api/chats/<id>/cancel` 返回 200，该对话**约 5 秒内停止**（事件 33 → 38），随后**排队者拿到运行权**（`waiting=1 → running=1`）。这两条此前只有单元测试与旧内核线的验收，现在同样绑到了当前候选。顺带把本轮与上一轮的"科研核心"一并接进一键套件：`acceptance-suite.py` 现含 **8 项**（文件面板与预览 / 界面上传 / PDF 预览 / 上传件进研究流程 / 预览自动刷新 / 分步方案与确认 / 科研核心 / 运行时行为），并支持 `--only=<关键字>` 单跑与 `--list` 列出，发布时一条命令即可复核全部面向。
- **仍存的缺口**：① 已闭合（候选 `6a6734e9cbc0802c` 实测主管 57 工具无 `subagent`、数据助手首个请求即 21 项角色表，见上一条）；② **不是缺口**：会话列表的原生分组交互属刻意边界，理由与证据见上一条，对等清单在 `dsh/IMPLEMENTATION.md`；③ 文件面板的实时变更流**已闭合**（契约补全、根失效与旧地址均已修，可见的自动刷新在运行中的候选上实测约 4 秒生效、无错误）；④ 正式发布尚未执行：技术链路已预演通过（主树冻结产出的候选与已验证的那份同 id 同字节，且源码与冻结清单 187/187 一致），生产仍是 `9e061099f3ccf519`，主树依赖树仍是 0.1.2（`check-env` 报 `DSH pin` 与 `Resolved DSH dependency graph` 两条失败、217 个包不匹配，但离线安装用的锁文件已自洽）；发布前仍需按 [dsh/RELEASES.md](dsh/RELEASES.md) 的「升级 DSH 内核依赖」升级主树依赖，而升级会替换当前管理员会话所依赖的运行时，因此与发布一并等明确许可后按序执行。
